...
def load_pc_file(filename):
    ...
    if pc.shape[0] != 4096 * dim:
        print("Error in pointcloud shape")
        print(pc.shape)
        print(filename)
        # return np.array([])
        return np.zeros([4096, dim])

    pc = np.reshape(pc, (pc.shape[0] // dim, dim))

    # preprocessing data
    # Normalization
    pc[:, 3:12] = ((pc - pc.min(axis=0)) / (pc.max(axis=0) - pc.min(axis=0)))[:, 3:12]      #  △△△ 这里的维度也要更换 △△△
    pc[np.isnan(pc)] = 0.0
    pc[np.isinf(pc)] = 1.0
...
def get_query_tuple(dict_value, num_pos, num_neg, QUERY_DICT, hard_neg=[], other_neg=False):
    ...
    query = load_pc_file(dict_value["query"])   # Nx13      加载点云数据

    random.shuffle(dict_value["positives"])     # 打乱顺序
    pos_files = []
    ...
    positives = load_pc_files(pos_files)        # 加载点云数据
